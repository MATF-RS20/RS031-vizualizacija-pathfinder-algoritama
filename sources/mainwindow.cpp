...
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent), ui(new Ui::MainWindow)
{
    ...
    connect(clear, SIGNAL(released()), this, SLOT(Clear()));

    // povezi start button
    QPushButton *start = MainWindow::findChild<QPushButton *>("btnStart");
    connect(start, SIGNAL(released()), this, SLOT(StartPressed()));
...
void MainWindow::Matrica()
{
    ...
    for (i = 0; i < red; i++)
    {
        for (j = 0; j < kolona; j++)
        {
            ...
            button[i][j]->show();
            connect(button[i][j], SIGNAL(released()), this, SLOT(ButtonPressed()));
...
void MainWindow::ButtonPressed()
{
    ...
    if (pocetni->isChecked())
        SetStart((x.ry() - 20) / velicinaDugmeta, (x.rx() - 30) / velicinaDugmeta);
    else if (krajnji->isChecked())
        SetEnd((x.ry() - 20) / velicinaDugmeta, (x.rx() - 30) / velicinaDugmeta);
    else
        SetPrepreka((x.ry() - 20) / velicinaDugmeta, (x.rx() - 30) / velicinaDugmeta);
}
...
void MainWindow::Iscrtaj()
{
    ...
    if (start[0] >= red - 1)
        SetStart(red - 1, start[1]);
    if (start[1] >= kolona - 1)
        SetStart(start[0], kolona - 1);
...
void MainWindow::StartPressed()
{
    ...
    {
        QMessageBox::information(this, tr(""), tr("Molim vas odaberite pocetno polje"));
        return;
    }
    if (end[0] == -1 || end[1] == -1)
    {
        QMessageBox::information(this, tr(""), tr("Molim vas odaberite krajnje polje"));
        return;
    }
    // soft ciscenje
    SoftClear();
...
void MainWindow::Animiraj()
{
    if (!objekat->korak->isActive())
    {
        ...
        else
        {
            if (!pronadjen_put)
            {
                QMessageBox::information(this, tr(""), tr("Put ne postoji."));
            }
            timer->stop();
        }
    }
